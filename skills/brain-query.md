---
name: brain-query
description: 先自查当前会话与草稿热记忆，仅在事实不足时降级检索中央知识库，并回写命中笔记激活信息与查询日志
allowed-tools:
  - Read
  - Bash(npm *)
  - Bash(git *)
...
- 只要其中任一处已经存在可直接支撑回答的事实，就直接回答。
- 一旦命中，必须停止所有检索执行下一步。

### 2. 第二级：降级召回（中央记忆仓库外求）

- 只有在第一级明确失败后，才允许调用 `bm25_search.py`。
- 第二级的目标是补足缺失事实，不是重复验证当前会话里已经明确存在的内容。

## 成功标准

- 已先完成第一级自查，并且仅在自查失败后才进入第二级。
- 若第一级已命中，则 0 次本地终端命令、0 次中央知识库检索。
- 实际运行了 `bm25_search.py`。
- 回答前已读完 Top 3 命中文件全文。
- 回答可追溯到命中文件。
- 已更新命中文件 frontmatter。
...
- 禁止跳过第一级自查，直接搜索中央知识库。
- 禁止在当前会话或当前草稿已存在明确事实时，仍调用任何本地终端命令。
- 禁止把中央知识库当作默认入口；它只能是第一级失败后的降级路径。
- 禁止绕过 `bm25_search.py` 直接全库扫描。
- 禁止在未读完命中文件前回答。
- 禁止把未命中的文件当作本次依据。
- 禁止结果为空时仍声称找到了相关知识。

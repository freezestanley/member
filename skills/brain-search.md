---
name: brain-search
description: 先自查当前会话热记忆，仅在事实不足时跨公共笔记、项目私有笔记与历史会话做资产分布审计，并回写命中笔记激活信息与查询日志
allowed-tools:
  - Read
  - Bash(npm *)
  - Bash(git *)
...
严格执行"非必要不求助"漏斗模型：先检查自身会话上下文与即时缓冲区；只有在自身没有相关事实时，才允许检索中央知识库与历史会话，用来盘点某个关键词在公共规范、项目专属笔记和历史讨论中的分布。

它的目标是"找资产、看分布、做审计"；如果用户要基于知识库给出精准答案，优先使用 `/brain-query`。
...
5. 获取当前项目名：
   ...
   ```
6. 根据 `--scope` 值做本地 Wiki BM25 检索：
   - `--scope global`（默认）：
     ```bash
     python3 /Users/za-stanlexu/Documents/member/member/scripts/bm25_search.py "<检索词>"
     ```
   - `--scope project`：
     ```bash
     python3 /Users/za-stanlexu/Documents/member/member/scripts/bm25_search.py "<检索词>" --project <当前项目名>
     ```
...
11. 更新 `wiki/log.md`（调用脚本，禁止手动写入）：
   ...
   python3 /Users/za-stanlexu/Documents/member/member/scripts/log_append.py \
     "brain-search" "<查询词>" "<命中N条：一句话不超过30字概括结果>"
   ```
...
- 只有在第一级明确失败后，才允许调用本地 Wiki 检索与 MemPalace 检索。
- 第二级的目标是补足缺失事实，不是重复验证当前会话里已经明确存在的内容。

## 成功标准

- 已先完成第一级自查，并且仅在自查失败后才进入第二级。
- 若第一级已命中，则 0 次本地 Wiki 检索、0 次 MemPalace 检索。
- 实际运行了本地 Wiki 检索命令。
...
- 禁止把 `/brain-search` 当作 `/brain-query` 的替代品直接输出确定性结论。
